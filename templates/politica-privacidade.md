# Template — Política de Privacidade

 Este é um modelo base. Adapte conforme a realidade da sua empresa e consulte um especialista jurídico, para que seja preenchido de maneira correta 

---

# Política de Privacidade — [Nome da Empresa]

**Última atualização:** [Data]

## 1. Quem somos
[Nome da empresa], inscrita sob o CNPJ [número], com sede em [endereço], é responsável pelo tratamento dos seus dados pessoais conforme esta Política de Privacidade.

## 2. Quais dados coletamos
- Dados de identificação: nome, CPF, e-mail, telefone
- Dados de navegação: IP, cookies, páginas acessadas
- [Adicionar outros dados conforme a realidade da empresa]

## 3. Por que coletamos seus dados
Utilizamos seus dados para:
- [Finalidade 1 — ex: prestar os serviços contratados]
- [Finalidade 2 — ex: enviar comunicações sobre nossos produtos]
- [Finalidade 3 — ex: cumprir obrigações legais]

## 4. Base legal
O tratamento dos seus dados é realizado com base em:
- Consentimento (Art. 7º, I)
- Execução de contrato (Art. 7º, V)
- Obrigação legal (Art. 7º, II)

## 5. Com quem compartilhamos seus dados
Seus dados podem ser compartilhados com:
- Parceiros e fornecedores necessários para a prestação dos serviços
- Autoridades públicas, quando exigido por lei

Não vendemos seus dados pessoais.

## 6. Por quanto tempo guardamos seus dados
Mantemos seus dados pelo tempo necessário para cumprir as finalidades descritas nesta política ou conforme exigido por lei.

## 7. Seus direitos
Você pode, a qualquer momento:
- Confirmar se tratamos seus dados
- Acessar, corrigir ou excluir seus dados
- Revogar seu consentimento
- Solicitar portabilidade dos dados

Para exercer seus direitos, entre em contato: [e-mail do DPO]

## 8. Encarregado (DPO)
**Nome:** [Nome do DPO]
**E-mail:** [e-mail]

## 9. Alterações nesta política
Esta política pode ser atualizada periodicamente. Notificaremos sobre mudanças relevantes.
